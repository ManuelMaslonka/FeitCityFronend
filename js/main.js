...
(function ($) {
    ...
    if ($.fn.countdown !== undefined && $.fn.countdown !== null) {
        $('.countdown').each(function () {
            ...
            // Function to initialize countdown when data is available
            function initCountdown() {
                // Get date from element's content
                var endDateValue = _this.text().trim();

                // Only initialize if we have an actual date value
                if (endDateValue && endDateValue !== "undefined" && endDateValue !== "null" && endDateValue !== "") {
                    // Clear the inner content for the countdown
                    _this.empty();

                    _this.countdown(endDateValue).on('update.countdown', function (event) {
                        _this.html(event.strftime('' + '<div class="counter-container"><div class="countdown-box first"><div class="number">%-D</div><span>Dní</span></div>' + '<div class="countdown-box"><div class="number">%H</div><span>Hodín</span></div>' + '<div class="countdown-box"><div class="number">%M</div><span>Minút</span></div>' + '<div class="countdown-box last"><div class="number">%S</div><span>Sekúnd</span></div></div>'));
            ...
            // Check if there's already data in the element
            if (_this.text().trim() !== "") {
                initCountdown();
            } else {
                // Set up a mutation observer to wait for content to be added by Alpine
                ...
                observer.observe(_this[0], {childList: true, subtree: true, characterData: true});

                // Fallback - try again after 1 second if data hasn't arrived
                setTimeout(function () {
                    if (_this.text().trim() !== "") {
                        initCountdown();
                    }
